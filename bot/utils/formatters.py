...
def format_codes_as_html_table(codes: list[dict[str, Any]]) -> str:
    ...
    lines = []
    # Используем <code> для моноширинного шрифта, что гарантирует ровность таблицы
    for c in codes[:MAX_CODES_DISPLAY]:
        code = c.get("code", "?").ljust(12)
        role = c.get("role", "?").ljust(12)
    ...
    # Собираем финальное сообщение в тегах <pre> для сохранения форматирования
    table = f"<pre>{header}\n{'-' * len(header)}\n{table_body}</pre>"

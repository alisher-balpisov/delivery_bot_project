...
class OrdersClient(BaseApiClient):
    ...
    async def create_order(self, token: str, order_data: dict) -> Optional[Dict[str, Any]]:
        """Создать новый заказ."""
        return await self._make_request(
            "POST", "/orders/", token=token, json_data=order_data, expected_status=201
        )

    async def get_order_details(self, token: str, order_id: int) -> Optional[Dict[str, Any]]:
        """Получить детали заказа по ID."""
        return await self._make_request("GET", f"/orders/{order_id}", token=token)

    async def get_user_orders(self, token: str) -> Optional[List[Any]]:
    ...
    async def get_available_orders(self, token: str) -> Optional[List[Any]]:

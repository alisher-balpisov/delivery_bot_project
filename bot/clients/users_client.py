...
class UsersClient(BaseApiClient):
    ...
    async def get_user_profile(self, token: str) -> dict[str, Any] | None:
        """Получить профиль пользователя по токену."""
        return await self._make_request("GET", "/users/me", token=token)

    async def get_all_couriers(self, token: str) -> list[Any] | None:
    ...
    async def update_user_info(
        self, token: str, telegram_id: int, update_data: dict
    ) -> dict[str, Any] | None:

...
class DisputesClient(BaseApiClient):
    ...
    async def create_dispute(self, telegram_id: int, dispute_data: dict) -> dict[str, Any] | None:
        """Создать новый спор."""
    ...
    async def get_dispute_details(self, telegram_id: int, dispute_id: int) -> dict[str, Any] | None:
    ...
    async def get_my_disputes(self, telegram_id: int) -> list[Any] | None:
